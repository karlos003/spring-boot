...
<div class="box">
    <div class="content">
        <div id="head">
            ...
            <a href="<%=basePath %>uprofile/${requestScope.post.user_account }" target="_blank"><img alt=" "
                                                                                                     src="<%=basePath%>u${requestScope.post.user_account}.jpg"
                                                                                                     width="30"
                                                                                                     height="30"></a>
            <a href="<%=basePath %>uprofile/${requestScope.post.user_account }"
               target="_blank">${requestScope.post.user_name }</a>
            ...
            <c:if test="${requestScope.post != null }">
                <a onclick="return loginCheck()"
                   href="<%=basePath%>postController/addLikeNum?post_id=${requestScope.post.post_id}">点赞</a>&nbsp;&nbsp;
                <a onclick="return loginCheck()"
                   href="<%=basePath%>userController/addSubsPost?post_id=${requestScope.post.post_id}">收藏</a>&nbsp;&nbsp;
        ...
        <c:forEach begin="1" end="${requestScope.post.post_image }" var="i">
            <a href="<%=basePath%>p${requestScope.post.post_id }_${i }.jpg" target="_blank">
                <img alt="图片..." src="<%=basePath%>p${requestScope.post.post_id }_${i }.jpg"
                     onload="AutoResizeImage(900,700,this);">
            </a>
            <br/>
        </c:forEach>
        ...
        <c:if test="${requestScope.post != null }">
            <!-- 评论输入栏 -->
            <div id="commentInput">
                <form action="<%=basePath %>commentController/addPostComment?post_id=${requestScope.post.post_id}"
                      onsubmit="return valid()" enctype="multipart/form-data" method="post">
